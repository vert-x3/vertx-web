...
To https://vertx.io/docs/vertx-core/java/#golden_rule[avoid blocking the event loop], all you have to do is implement https://www.graphql-java.com/documentation/${graphql.java.doc.version}/data-fetching/[data fetchers] that return a `CompletableFuture` instead of the result directly.

[source,java]
----
{@link examples.GraphQLExamples#completableFutureDataFetcher}
----

=== Fetching data with callback-based APIs

Implementing a data fetcher that return a `CompletableFuture` is not a complex task.
