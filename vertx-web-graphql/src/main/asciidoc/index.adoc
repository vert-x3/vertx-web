...
You can use an Apollo WebSocketLink which connects over a websocket. This is specially useful if you want
...
The Apollo client has to be configured to consume the HTTP endpoint configured with the ApolloWsHandler
as WebSocket and it will be upgrade. If you want to support a WebSocketLink and a HttpLink in
the same path, you can add the ApolloWSHandler in first place and then the GraphQLHandler.

This is an example of how to configure the Apollo SubscriptionClient:
...
execute(link, { query: query }).subscribe(r => console.log(r));
----

A subscription DataFetcher has to return a Publisher instance.

----
{@link examples.GraphQLExamples#subscriptionDataFetcher}
----
