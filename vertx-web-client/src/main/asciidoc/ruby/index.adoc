...
----
require 'vertx-web-client/body_codec'
client.get(8080, "myserver.mycompany.com", "/some-uri").send(VertxWebClient::BodyCodec.write_stream(writeStream)) { |ar_err,ar|
  if (ar_err == nil)
...
Finally if you are not interested at all by the response content, the `link:../../yardoc/VertxWebClient/BodyCodec.html#none-class_method[BodyCodec.none]` will
