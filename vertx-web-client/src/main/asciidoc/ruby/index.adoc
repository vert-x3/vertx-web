...
single = client.get(8080, "myserver.mycompany.com", "/some-uri").rx_send()

# Send a request upon subscription of the Single
single.subscribe(lambda { |response|
  puts "Received 1st response with status code#{response.status_code()}"
}, lambda { |error|
  puts "Something went wrong #{error.get_message()}"
})

# Send another request
single.subscribe(lambda { |response|
  puts "Received 2nd response with status code#{response.status_code()}"
}, lambda { |error|
  puts "Something went wrong #{error.get_message()}"
})
...
url = client.get(8080, "myserver.mycompany.com", "/some-uri").rx_send().map(Java::IoVertxRxjavaExtWebClient::HttpResponse.method(:body_as_string))

# Use the flatMap operator to make a request on the URL Single
url.flat_map(lambda { |u|
  client.get_abs(u).rx_send()
}).subscribe(lambda { |response|
  puts "Received response with status code#{response.status_code()}"
}, lambda { |error|
  puts "Something went wrong #{error.get_message()}"
})
...
[source,ruby]
----
single = client.get(8080, "myserver.mycompany.com", "/some-uri").put_header("some-header", "header-value").add_query_param("some-param", "param value").as(Java::IoVertxRxjavaExtWebCodec::BodyCodec.json_object()).rx_send()
single.subscribe(lambda { |resp|
...
single = client.post(8080, "myserver.mycompany.com", "/some-uri").rx_send_stream(body)
single.subscribe(lambda { |resp|
  puts resp.status_code()
  puts resp.body()
})
