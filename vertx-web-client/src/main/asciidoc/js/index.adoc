...
single.subscribe(function (response) {
  console.log("Received 1st response with status code" + response.statusCode());
}, function (error) {
  console.log("Something went wrong " + error.getMessage());
});

// Send another request
single.subscribe(function (response) {
  console.log("Received 2nd response with status code" + response.statusCode());
}, function (error) {
  console.log("Something went wrong " + error.getMessage());
});
...
var url = client.get(8080, "myserver.mycompany.com", "/some-uri").rxSend().map(Java.type("io.vertx.rxjava.ext.web.client.HttpResponse").bodyAsString);

// Use the flatMap operator to make a request on the URL Single
url.flatMap(function (u) {
...
[source,js]
----
var single = client.get(8080, "myserver.mycompany.com", "/some-uri").putHeader("some-header", "header-value").addQueryParam("some-param", "param value").as(Java.type("io.vertx.rxjava.ext.web.codec.BodyCodec").jsonObject()).rxSend();
single.subscribe(function (resp) {
