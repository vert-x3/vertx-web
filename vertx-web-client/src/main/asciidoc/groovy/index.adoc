...
When large response are expected, use the `link:../../apidocs/io/vertx/webclient/BodyCodec.html#writeStream-io.vertx.core.streams.WriteStream-[BodyCodec.writeStream]`.
...
----
import io.vertx.webclient.BodyCodec
client.get(8080, "myserver.mycompany.com", "/some-uri").send(BodyCodec.writeStream(writeStream), { ar ->
  if (ar.succeeded()) {
