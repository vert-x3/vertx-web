...
def single = client.get(8080, "myserver.mycompany.com", "/some-uri").rxSend()

// Send a request upon subscription of the Single
single.subscribe({ response ->
  println("Received 1st response with status code${response.statusCode()}")
}, { error ->
  println("Something went wrong ${error.getMessage()}")
})

// Send another request
single.subscribe({ response ->
  println("Received 2nd response with status code${response.statusCode()}")
}, { error ->
  println("Something went wrong ${error.getMessage()}")
})
...
def url = client.get(8080, "myserver.mycompany.com", "/some-uri").rxSend().map(io.vertx.rxjava.ext.web.client.HttpResponse.&bodyAsString)
...
[source,groovy]
----
def single = client.get(8080, "myserver.mycompany.com", "/some-uri").putHeader("some-header", "header-value").addQueryParam("some-param", "param value").as(io.vertx.rxjava.ext.web.codec.BodyCodec.jsonObject()).rxSend()
...
def body = this.getPayload()

def single = client.post(8080, "myserver.mycompany.com", "/some-uri").rxSendStream(body)
single.subscribe({ resp ->
  println(resp.statusCode())
  println(resp.body())
})
