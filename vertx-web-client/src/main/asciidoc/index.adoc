...
A custom predicate instance may be created with {@link io.vertx.ext.web.client.predicate.ResponsePredicate#create(Function)} and then activated with {@link io.vertx.ext.web.client.HttpRequest#expect(ResponsePredicate)}.
...
You may customize the exception class by changing the error converter:
...
But many web APIs provide details in error responses.
...
To avoid losing this information, it is possible to wait for the response body to be fully received before the error converter is invoked:
