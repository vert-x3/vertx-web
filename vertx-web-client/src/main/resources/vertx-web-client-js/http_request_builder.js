...
var HttpResponseBuilder = require('vertx-web-client-js/http_response_builder');
...
var HttpRequestBuilder = function(j_val) {
  ...
  /**
   Configure to buffer the body and returns a [< Buffer >] {@link HttpResponseBuilder} for further configuration of
   the response or [#send(Handler) sending] {@link HttpResponseBuilder} the request.
  ...
  this.bufferBody = function() {
    var __args = arguments;
    if (__args.length === 0) {
      return utils.convReturnVertxGen(HttpResponseBuilder, j_httpRequestBuilder["bufferBody()"](), Buffer._jtype);
    } else throw new TypeError('function invoked with invalid arguments');
  };
