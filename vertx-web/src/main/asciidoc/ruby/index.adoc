...
route = router.route_with_regex("\\/(?<productType>[^\\/]+)\\/(?<productId>[^\\/]+)").handler() { |routingContext|
...
authProvider = VertxAuthOauth2::GithubAuth.create(vertx, "YOUR PROVIDER CLIENTID", "YOUR PROVIDER CLIENT SECRET")
...
router.get("/protected").handler() { |ctx|
  ...
  ctx.response().end(user.to_string())
}
