...
Sessions are saved in a best effort, there are occasions when it can make sense to have guarantees that the sessions
have been flushed to the underlying store before completing the response. In this case you can force a flush. Note that
doing this will disable the automatic save at the end only and only if the flush was successful. This will allow you
to control the the state before completing the response like:
