...
The routing context {@link io.vertx.ext.web.RoutingContext#data()} is available
in the Pebble template as the source of variables, this means you can render the template based on anything in the context
data. If you need data from the request, response, session then it needs to be added explicitly to the data.
