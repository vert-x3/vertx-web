...
To use the Pug template engine, you need to add the following _dependency_ to your project:
`${maven.groupId}:vertx-web-templ-pug:${maven.version}`. Create an instance of the Pug template engine using:
`io.vertx.ext.web.templ.pug.PugTemplateEngine#create(io.vertx.core.Vertx)`.

When using the Pug template engine, it will by default look for
templates with the `.pug` extension if no extension is specified in the file name.

The routing context {@link io.vertx.ext.web.RoutingContext#data()} is available
in the Pug template as the source of variables, this means you can render the template based on anything in the context
...
Please consult the https://github.com/neuland/pug4j[Pug4j documentation] for how to write
Pug templates.
