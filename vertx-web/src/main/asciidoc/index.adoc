...
To use the Jade template engine, add the following dependency to the _dependencies_ section of your build descriptor:
...
----
dependencies {
  compile 'io.vertx:vertx-web-templ-jade:${maven.version}'
}
----

Create an instance of the Jade template engine using: `io.vertx.ext.web.templ.jade.JadeTemplateEngine#create(io.vertx.core.Vertx)`.
...
html
  head
    //- foo is an entry within routing context data
    title= foo
  body
----
