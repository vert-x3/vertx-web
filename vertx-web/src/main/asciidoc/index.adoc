...
Vert.x Web is designed to be powerful, un-opionated and fully embeddable.
You just use the parts you want and nothing more.
Vert.x Web is not a container.

You can use Vert.x Web to create classic server-side web applications, RESTful web applications, 'real-time' (server push) web applications, or any other kind of web application you can think of.
Vert.x Web doesn't care.
It's up to you to choose the type of app you prefer, not Vert.x Web.

Vert.x Web is a great fit for writing RESTful HTTP microservices, **but we don't force you** to write apps like that.
...
Most features in Vert.x Web are implemented as handlers so you can always write your own.
We envisage many more being
written over time.
...
Here's a hello world web server written using Vert.x core.
At this point there is no Vert.x Web involved:
...
A {@link io.vertx.ext.web.Router} is one of the core concepts of Vert.x Web.
It's an object which maintains zero or more
...
The object that gets passed into the handler is a {@link io.vertx.ext.web.RoutingContext} - this contains the standard Vert.x {@link io.vertx.core.http.HttpServerRequest} and {@link io.vertx.core.http.HttpServerResponse} but also various other useful stuff that makes working with Vert.x Web simpler.
...
[CAUTION]
====
Uploads can be a source of DDoS attacks.
In order to reduce the attack surface, it is recommended to set sensible limits with {@link io.vertx.ext.web.handler.BodyHandlerOptions#setBodyLimit}.
For example, 10mb for general uploads or 100kb for JSON.
====
...
To limit the size of a request body, create the body handler then use {@link io.vertx.ext.web.handler.BodyHandlerOptions#setBodyLimit(long)} to specify the maximum body size, in bytes.
This is useful to avoid running out of memory with very large bodies.
...
By default, the body size is limited to 10 megabytes.

=== Merging form attributes

By default, the body handler will merge any form attributes into the request parameters.
If you don't want this behaviour you can use disable it with {@link io.vertx.ext.web.handler.BodyHandlerOptions#setMergeFormAttributes(boolean)}.

=== Handling file uploads

Body handler is also used to handle multipart file uploads.
...
=== Housekeeping the uploaded files directory

The {@link io.vertx.ext.web.handler.BodyHandler} can be configured to clean the uploaded files directory:

[source,$lang]
----
{@link examples.WebExamples#housekeepingUploadDir}
----

This will delete any file uploaded as part of a request when the corresponding response has been sent.
...
Vert.x Web uses session cookies to identify a session.
The session cookie is temporary and will be deleted by your browser
when it's closed.
...
If two users share same session id, they share same session variables too and website is considering them as one visitor.
This could be a security risk if session is used for any private or sensitive data, or to allow access to restricted areas of website.
When cookies are used, session id can be protected using SSL and by marking a cookie as
...
By default, the session handler will always add a session cookie to the HTTP response, even if the session has not been accessed by your application.
To create the session cookie only when the session has been used, use
{io.vertx.ext.web.handler.SessionHandlerOptions#setLazySession(boolean)}.
...
Authentication provider is used for authentication of users.
Vert.x provides several authentication provider instances out-of-the-box in the vertx-auth project.
For full information on auth providers and how to use and configure them
...
You will also need to set up handlers to serve your actual login page, and a handler to handle the actual login itself.
...
Vert.x Web comes with an out-of-the-box handler for auditing security events.
This handler is useful for logging to an external system like EDR/XDR.
EDR stands for Endpoint Detection and Response and XDR stands for Extended Detection and Response.
These are security products that are used to detect and respond to threats on endpoints.
The handler is called `SecurityAuditLoggerHandler` and it used as a top level handler as:

[source,$lang]
----
{@link examples.WebExamples#example88}
----

=== Event types

The handler will log the following events:

==== REQUEST

When a request is received (either success or fail) you will get a log event with the following information:

----
[REQUEST epoch="1679389193762" source="127.0.0.1" destination="127.0.0.1" resource="HTTP/1.1 GET /protected/foo" status=200] OK
----

When a failure occurs you will get a log event with the following information:
...
* epoch - the time in milliseconds since the epoch
...
When a request is authenticated you will get a log event with the following information:

----
[AUTHENTICATION epoch="1679389193762" source="127.0.0.1" destination="127.0.0.1" resource="HTTP/1.1 GET /protected/foo" token="********************************..."] OK
----

The handler is masking well known PII fields like passwords, tokens, etc. Also for failures there will be an event
logged:
...
When a request is authorized you will get a log event with the following information:
...
When a request is not authorized you will get a log event with the following information:

----
[AUTHORIZATION epoch="1679389193678" source="127.0.0.1" destination="127.0.0.1" resource="HTTP/1.1 GET /protected/page1" subject="paulo" authorization="ROLE[role3]"] OK
----

The the event will include a couple of extra fields:

* subject - the subject that was authenticated
* authorization - the authorization that was *required* to authorize the request

The logger will not include user specific authorization information to avoid leaking PII/PHI information.

=== Configuration

The handler will usually log using rfc5424 format (usually known as syslog) but you can configure it to use other
formats. The configuration is global as it affects the logging format, so to change it you need to do it at a system
property defined at startup time:
...
When using JSON, the same data is logged in a single line json document.
...
To serve static resources such as `.html`, `.css`, `.js` or any other static resource, you use an instance of {@link io.vertx.ext.web.handler.StaticHandler}.
...
It will also look for a file on the classpath called `webroot/css/mystyle.css`.
This means you can package up all your static resources into a jar file (or fatjar) and distribute them like that.
...
The handler will handle range aware requests.
When a client makes a request to a static resource, the handler will notify that it can handle range aware request by stating the unit on the `Accept-Ranges` header.
Further requests that contain the `Range` header with the correct unit and start and end indexes will then receive partial responses with the correct `Content-Range` header.
...
`cache-control` is set to `max-age=86400` by default.
This corresponds to one day.
This can be configured with
{@link io.vertx.ext.web.handler.StaticHandlerOptions#setMaxAgeSeconds(long)} if required.
...
If handling of cache headers is not required, it can be disabled with {@link io.vertx.ext.web.handler.StaticHandlerOptions#setCachingEnabled(boolean)}.
...
If you know that your files never change on disk, then the cache entry will effectively never expire.
This is the default.
...
{@link io.vertx.ext.web.handler.StaticHandlerOptions#setFilesReadOnly(boolean)}.

To enable the maximum number of entries that can be cached in memory at any one time you can use
{@link io.vertx.ext.web.handler.StaticHandlerOptions#setMaxCacheSize(int)}.

To configure the expiry time of cache entries you can use {@link io.vertx.ext.web.handler.StaticHandlerOptions#setCacheEntryTimeout(long)}.

=== Configuring the index page

Any requests to the root path `/` will cause the index page to be served.
By default, the index page is `index.html`.
This can be configured with {@link io.vertx.ext.web.handler.StaticHandlerOptions#setIndexPage(String)}.

=== Changing the web root

By default, static resources will be served from the directory `webroot`.
To configure this use {@link io.vertx.ext.web.handler.StaticHandler#create(String)}.

=== Serving hidden files

By default, the serve will serve hidden files (i.e. files starting with `.`).

If you do not want hidden files to be served you can configure it with {@link io.vertx.ext.web.handler.StaticHandlerOptions#setIncludeHidden(boolean)}.

=== Directory listing

The server can also perform directory listing.
By default, directory listing is disabled.
To enabled it use
{@link io.vertx.ext.web.handler.StaticHandlerOptions#setDirectoryListing(boolean)}.
...
{@link io.vertx.ext.web.handler.StaticHandlerOptions#setDirectoryTemplate(String)}.

=== Disabling file caching on disk

By default, Vert.x will cache files that are served from the classpath into a file on disk in a sub-directory of a directory called `.vertx` in the current working directory.
This is mainly useful when deploying services as fatjars in production where serving a file from the classpath every time can be slow.
...
To disable file caching you can provide your vert.x options the property `fileResolverCachingEnabled` to `false`.
For backwards compatibility it will also default that value to the system property `vertx.disableFileCaching`.
E.g. you could set up a run configuration in your IDE to set this when running your main class.
...
By default, the template handler will look for templates in a directory called `templates`.
...
not embedded in vertx-web, so you must configure your project to access them. Configuration is provided for
each template engine.

Here's an example with the Handlebars template engine:

////
These examples are not using the traditional "transcoding" as they use an API provided in another project.
////
...
To use the MVEL template engine, add the following dependency to the _dependencies_ section of your build descriptor:
...
----
dependencies {
  compile 'io.vertx:vertx-web-templ-mvel:${maven.version}'
}
----

Create an instance of the MVEL template engine using: `io.vertx.ext.web.templ.mvel.MVELTemplateEngine#create(io.vertx.core.Vertx)`
...
The routing context {@link io.vertx.ext.web.RoutingContext#data()} is available in the MVEL template as the source of variables, this means you can render the template based on anything in the context data.
If you need data from the request, response, session then it needs to be added explicitly to the data.
...
To use the Pug template engine, add the following dependency to the _dependencies_ section of your build descriptor:
...
----
dependencies {
  compile 'io.vertx:vertx-web-templ-pug:${maven.version}'
}
----

Create an instance of the Pug template engine using: `io.vertx.ext.web.templ.pug.PugTemplateEngine#create(io.vertx.core.Vertx)`.

When using the Pug template engine, it will by default look for templates with the `.pug` extension if no extension is specified in the file name.

The routing context {@link io.vertx.ext.web.RoutingContext#data()} is available in the Pug template as the source of variables, this means you can render the template based on anything in the context data.
If you need data from the request, response, session then it needs to be added explicitly to the data.
...
Please consult the https://github.com/neuland/pug4j[Pug4j documentation] for how to write Pug templates.

=== Handlebars template engine

To use the Handlebars template engine, add the following dependency to the _dependencies_ section of your build descriptor:
...
----
dependencies {
  compile 'io.vertx:vertx-web-templ-handlebars:${maven.version}'
}
----

Create an instance of the Handlebars template engine using: `io.vertx.ext.web.templ.handlebars.HandlebarsTemplateEngine#create(io.vertx.core.Vertx)`.
...
The routing context {@link io.vertx.ext.web.RoutingContext#data()} is available in the Handlebars template as the source of variables, this means you can render the template based on anything in the context data.

[IMPORTANT]
====
Handlebars templates are not able to call arbitrary methods in objects.

If you want to have access to other data like the request path, request params or session data you should add it the context data in a handler before the template handler.
For example:
...
Please consult the https://github.com/jknack/handlebars.java[Handlebars Java port documentation] for how to write handlebars templates.

=== Thymeleaf template engine

To use the Thymeleaf template engine, add the following dependency to the _dependencies_ section of your build descriptor:
...
----
dependencies {
  compile 'io.vertx:vertx-web-templ-thymeleaf:${maven.version}'
}
----

Create an instance of the Thymeleaf template engine using: `io.vertx.ext.web.templ.thymeleaf.ThymeleafTemplateEngine#create(io.vertx.core.Vertx)`.
...
The routing context {@link io.vertx.ext.web.RoutingContext#data()} is available in the Thymeleaf template as the source of variables, this means you can render the template based on anything in the context data.
If you need data from the request, response, session then it needs to be added explicitly to the data.
...
Please consult the http://www.thymeleaf.org/[Thymeleaf documentation] for how to write Thymeleaf templates.

=== Apache FreeMarker template engine

To use the FreeMarker template engine, add the following dependency to the _dependencies_ section of your build descriptor:
...
----
dependencies {
  compile 'io.vertx:vertx-web-templ-freemarker:${maven.version}'
}
----

Create an instance of the Apache FreeMarker template engine using: `io.vertx.ext.web.templ.Engine#create()`.

When using the Apache FreeMarker template engine, it will by default look for templates with the `.ftl` extension if no extension is specified in the file name.

The routing context {@link io.vertx.ext.web.RoutingContext#data()} is available in the FreeMarker template as the source of variables, this means you can render the template based on anything in the context data.
If you need data from the request, response, session then it needs to be added explicitly to the data.
...
Please consult the http://www.freemarker.org/[Apache FreeMarker documentation] for how to write Apache FreeMarker templates.

=== Pebble template engine

To use the Pebble template engine, add the following dependency to the _dependencies_ section of your build descriptor:
...
----
dependencies {
  compile 'io.vertx:vertx-web-templ-pebble:${maven.version}'
}
----

Create an instance of the Pebble template engine using: `io.vertx.ext.web.templ.pebble.PebbleTemplateEngine#create(vertx)`.

When using the Pebble template engine, it will by default look for templates with the `.peb` extension if no extension is specified in the file name.

The routing context {@link io.vertx.ext.web.RoutingContext#data()} is available in the Pebble template as the source of variables, this means you can render the template based on anything in the context data.
If you need data from the request, response, session then it needs to be added explicitly to the data.
...
Please consult the http://www.mitchellbosecke.com/pebble/home/[Pebble documentation] for how to write Pebble templates.

=== Rocker template engine

To use the Rocker template engine, add the following dependency to the _dependencies_ section of your build descriptor:
...
----
dependencies {
  compile 'io.vertx:vertx-web-templ-rocker:${maven.version}'
}
----

Create an instance of the Rocker template engine using: `io.vertx.ext.web.templ.rocker.RockerTemplateEngine#create()`.

The routing context {@link io.vertx.ext.web.RoutingContext#data()} is available in the Rocker template as the source of variables, this means you can render the template based on anything in the context data.
If you need data from the request, response, session then it needs to be added explicitly to the data.
...
To use the HTTL template engine, add the following dependency to the _dependencies_ section of your build descriptor:
...
----
dependencies {
  compile 'io.vertx:vertx-web-templ-httl:${maven.version}'
}
----

Create an instance of the HTTL template engine using: `io.vertx.ext.web.templ.httl.HTTLTemplateEngine#create(io.vertx.core.Vertx)`.

When using the HTTL template engine, it will by default look for templates with the `.httl` extension if no extension is specified in the file name.

The routing context {@link io.vertx.ext.web.RoutingContext#data()} is available in the HTTL template as the source of variables, this means you can render the template based on anything in the context data.
If you need data from the request, response, session then it needs to be added explicitly to the data.
...
Please consult the https://httl.github.io/en/[HTTL documentation] for how to write HTTL templates.

=== Rythm template engine

To use the Rythm template engine, add the following dependency to the _dependencies_ section of your build descriptor:
...
----
dependencies {
  compile 'io.vertx:vertx-web-templ-rythm:${maven.version}'
}
----

Create an instance of the Rythm template engine using: `io.vertx.ext.web.templ.rythm.RythmTemplateEngine#create(io.vertx.core.Vertx)`.

When using the Rythm template engine, it will by default look for templates with the `.html` extension if no extension is specified in the file name.

The routing context {@link io.vertx.ext.web.RoutingContext#data()} is available in the Rythm template as the source of variables, this means you can render the template based on anything in the context data.
If you need data from the request, response, session then it needs to be added explicitly to the data.
...
To use the JTE template engine, add the following dependency to the _dependencies_ section of your build descriptor:
...
----
dependencies {
  compile 'io.vertx:vertx-web-templ-jte:${maven.version}'
}
----

Create an instance of the JTE template engine using: `io.vertx.ext.web.templ.jte.JteTemplateEngine#create(io.vertx.core.Vertx, java.lang.String)`.

The routing context {@link io.vertx.ext.web.RoutingContext#data()} is available in the JTE template as the source of variables, this means you can render the template based on anything in the context data.
If you need data from the request, response, session then it needs to be added explicitly to the data.
...
`VERTXWEB_ENVIRONMENT` to `dev` or `development`.
...
If a message is _out-bound_ (i.e. being sent from the server to client side JavaScript) before it is sent to the client Vert.x Web will look through any outbound permitted matches.
If any match, it will be allowed through.
...
Again this callback will return status code 401 if there is no user already authenticated in the request. On success a JSON document is returned with a url, and a few extra metadata.
This url should be used to configure the authenticator, either by manually entering it on the application or by rendering a QR code.
Rendering of QR codes can be done both on the backend or on the frontend.
For the sake of simplicity, this example makes use of the Google charts API to render
it on the browser.
...
The link:/docs/vertx-health-check/java/[Vert.x Health Checks] module provides a way to determine the current state of an application.

Include it to your project's dependencies:
...
Then declare the {@link io.vertx.ext.web.healthchecks.HealthCheckHandler} to expose the computed application state over HTTP.

[source,$lang]
----
{@link examples.WebExamples#exposeHealthChecks}
----

NOTE: Using an asterisk is not strictly required.
It is only useful if you need to be able to query the state of single procedure, or of a single group.

It is not required to create a {@link io.vertx.ext.healthchecks.HealthChecks} instance beforehand.
Health check link:/docs/vertx-health-check/java/#_procedures[procedures] can be registered directly on the handler.

[source,$lang]
----
{@link examples.WebExamples#registerHealthCheckProcedureDirectly}
----

=== HTTP responses and JSON Output

When using the {@link io.vertx.ext.web.healthchecks.HealthCheckHandler}, the overall health check is retrieved using an HTTP request.

If no procedure is registered, the response is `204 - NO CONTENT`, indicating that the system is _UP_ but no procedure has been executed.
The response does not contain a payload.

If there is at least one procedure registered, this procedure is executed and the outcome status is computed.
The response would use the following status code:
...
A `checks` array is also given indicating the result of the different executed procedures.
If the procedure has reported additional data, the data is also given:
...
If a procedure throws an error or reports a failure (exception), the JSON document provides the `cause` in the `data` section.
If a procedure does not report back before a timeout, the indicated cause is `Timeout`.

=== Authentication

When using the {@link io.vertx.ext.web.healthchecks.HealthCheckHandler}, you can provide a {@link io.vertx.ext.auth.authentication.AuthenticationProvider} to authenticate the request.

[source,$lang]
----
{@link examples.WebExamples#exposeHealthChecksWithAuth}
----

Check out link:/docs/#authentication_and_authorisation[Vert.x Auth docs] for more details about available authentication providers.

The handler creates a JSON object containing:

* the request headers
* the request params
* the form params, if any
* the content as JSON, if any, and if the request set the content type to `application/json`.

The resulting object is passed to the auth provider to authenticate the request.
If the authentication failed, it returns a `403 - FORBIDDEN` response.
