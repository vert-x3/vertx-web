...
If the authentication handler has successfully authenticated the user it will inject a {@link io.vertx.ext.auth.user.User}
...
If there is no {@link io.vertx.ext.auth.user.User} in the current request, then it is assumed that no previous authentication has been
