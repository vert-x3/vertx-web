...
IMPORTANT: When using blockingHandler in combination with multipart forms, you must add an additional async handler BEFORE the blockingHandler
           in order to call `setEnableMultipart(true). This is demonstrated below:
[source,js]
----
router.route("/post/path").handler(function(ctx) {
    ctx.setEnableMultipart(true); // Set enable multipart processing
    ctx.next();                   // Tell vertx-web/Router to continue processing other handler (like our blockingHandler)
