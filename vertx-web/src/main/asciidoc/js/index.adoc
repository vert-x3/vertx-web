...
router.get("/awesome/:pathParam").handler(validationHandler.handle).handler(function (routingContext) {
  ...
  // Get parameters
  var parameterName = params.getQueryParameter("parameterName").getInteger();
  var formParameterName = params.getFormParameter("formParameterName").getString();
  var pathParam = params.getPathParameter("pathParam").getFloat();
...
[source,js]
----
var OpenAPI3RouterFactory = require("vertx-web-js/open_api3_router_factory");
...
OpenAPI3RouterFactory.createRouterFactoryFromFile(vertx, "src/main/resources/petstore.yaml", function (openAPI3RouterFactoryAsyncResult, openAPI3RouterFactoryAsyncResult_err) {
  if (openAPI3RouterFactoryAsyncResult_err == null) {
    ...
    routerFactory.addHandlerByOperationId("listPets", function (routingContext) {
      ...
      var failure = routingContext.failure();
      if (failure.getClass().getSimpleName() == 'ValidationException') {
        routingContext.response().setStatusCode(400).setStatusMessage("ValidationException thrown! " + failure.getErrorType().name()).end()}
    });
    ...
    routerFactory.addHandler('POST', "/pets", function (routingContext) {
      ...
      var params = routingContext.get("parsedParameters");
      var pet = params.getBody().getJsonObject();
