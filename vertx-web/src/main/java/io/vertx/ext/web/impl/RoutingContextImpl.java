...
public class RoutingContextImpl extends RoutingContextImplBase {
  ...
  private IntMap<Handler<Void>> headersEndHandlers;
  private IntMap<Handler<Void>> bodyEndHandlers;
  // clean up handlers
  private IntMap<Handler<AsyncResult<Void>>> endHandlers;
  ...
  private IntMap<Handler<Void>> getHeadersEndHandlers() {
    if (headersEndHandlers == null) {
      headersEndHandlers = new IntMap<Handler<Void>>();
  ...
  private IntMap<Handler<Void>> getBodyEndHandlers() {
    if (bodyEndHandlers == null) {
      bodyEndHandlers = new IntMap<Handler<Void>>();
      // order is important we we should traverse backwards
  ...
  private IntMap<Handler<AsyncResult<Void>>> getEndHandlers() {
    if (endHandlers == null) {
      // order is important we we should traverse backwards
      endHandlers = new IntMap<Handler<AsyncResult<Void>>>();
