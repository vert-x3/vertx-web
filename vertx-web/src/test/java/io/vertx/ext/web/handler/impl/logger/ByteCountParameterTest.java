...
public class ByteCountParameterTest {
  ...
  public void testByteCountParameter_NonZero() {
    ...
    ByteCountParameter test = new ByteCountParameter(true);
    assertThat(test.getValue(ctx), equalTo("321"));

    test = new ByteCountParameter(false);
    assertThat(test.getValue(ctx), equalTo("123"));
  }
  ...
  public void testByteCountParameter_Zero() {
    ...
    ByteCountParameter test = new ByteCountParameter(false);
    assertThat(test.getValue(ctx), equalTo("0"));

    test.setParName('b');
    assertThat(test.getValue(ctx), equalTo("-"));
  }
}
