...
* Last parameter must be a `Handler<AsyncResult<OperationResult>>`
* Second to last parameter must be a {@link io.vertx.ext.web.api.RequestContext}
...
When you receive a request at `TransactionService.getTransactionsList()` the generated service handler will automatically extract `from` and `to` parameter (if present) from {@link io.vertx.ext.web.api.RequestContext}. In `putTransaction()` we use the `body` parameter name to extract the json body.
...
[source,java]
----
void putTransaction(Transaction body, RequestContext context, Handler<AsyncResult<OperationResult>> resultHandler);
----
...
Now you can implement your service. The {@link io.vertx.ext.web.api.RequestContext} object contains headers and parameters maps.

To write the request you must call the `resultHandler` with an {@link io.vertx.ext.web.api.OperationResult}. To construct the {@link io.vertx.ext.web.api.OperationResult} you can use some handy methods like {@link io.vertx.ext.web.api.OperationResult#completedWithJson(io.vertx.core.buffer.Buffer)} or {@link io.vertx.ext.web.api.OperationResult#completedWithPlainText(io.vertx.core.buffer.Buffer)}
