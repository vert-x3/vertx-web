...
IMPORTANT: If you want to use `link:../../yardoc/VertxWeb/DesignDrivenRouterFactory.html#add_handler-instance_method[addHandler]` or `link:../../yardoc/VertxWeb/DesignDrivenRouterFactory.html#add_failure_handler-instance_method[addFailureHandler]` pay attention: You can provide a path only in OpenAPI styles (for example path `/hello/:param` doesn't work)
