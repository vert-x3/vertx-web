 ...
 * {@link io.vertx.ext.web.designdriven.openapi3.OpenAPI3RouterFactory}.
 * This class, as name says, is a router factory based on your OpenAPI 3 specification.
 * {@link io.vertx.ext.web.designdriven.openapi3.OpenAPI3RouterFactory} is intended to give you a really simple user
 * interface to use OpenAPI 3 support. It includes:
 *
 ...
 * * Lazy methods: operations (combination of paths and HTTP methods) are mounted in declaration order inside
 * specification
 * * Automatic mount of security validation handlers
 *
 * == Create a new router factory
 * To create a new router factory, you can use methods inside
 * {@link io.vertx.ext.web.designdriven.openapi3.OpenAPI3RouterFactory}:
 *
 * * {@link io.vertx.ext.web.designdriven.openapi3.OpenAPI3RouterFactory#createRouterFactoryFromFile(io.vertx.core
 * .Vertx, java.lang.String, io.vertx.core.Handler)} to create a router factory from local file
 * *
 ...
 *
 * This two functions takes as parameters an handler and a failure handler. You can, of course, **add multiple
 * handlers to same operation**, without overwrite the existing ones.
 *
 * For example:
 * [source,$lang]
 * ----
 ...
 * IMPORTANT: Usage of combination of path and HTTP method is allowed, but it's better to add operations handlers
 * with operationId, for performance reasons and to avoid paths nomenclature errors
 *
 * Now you can use parameter values as described in http://vertx
 * .io/docs/vertx-web/java/#_andling_parameters[vertx-web documentation]
 *
 * == Define security handlers
 * A security handler is defined by a combination of schema name and scope. You can mount only one security handler
 * for a combination.
 * For example:
 *
 * [source,$lang]
 ...
 * * It automatically mounts a 400 `Bad Request` handler that manages `ValidationException` (You can enable/disable
 * this feature via
 * {@link io.vertx.ext.web.designdriven.openapi3.OpenAPI3RouterFactory#enableValidationFailureHandler(boolean)})
 *
 * == Generate the router
