...
public class Route {
   ...
   * This method works just like {@link io.vertx.groovy.ext.apex.Route#handler} excepted that it will run the blocking handler on a different thread
  ...
  public Route blockingHandler(Handler<RoutingContext> requestHandler) {
    this.delegate.blockingHandler(new Handler<io.vertx.ext.apex.RoutingContext>() {
      public void handle(io.vertx.ext.apex.RoutingContext event) {
        requestHandler.handle(new io.vertx.groovy.ext.apex.RoutingContext(event));
      }
    });
