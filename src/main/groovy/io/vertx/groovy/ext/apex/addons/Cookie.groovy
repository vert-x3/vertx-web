...
@CompileStatic
public class Cookie {
  final def io.vertx.ext.apex.addons.Cookie delegate;
  public Cookie(io.vertx.ext.apex.addons.Cookie delegate) {
  ...
  public static Cookie cookie(String name, String value) {
    def ret= Cookie.FACTORY.apply(io.vertx.ext.apex.addons.Cookie.cookie(name, value));
    return ret;
  }
  /**
  ...
  static final java.util.function.Function<io.vertx.ext.apex.addons.Cookie, Cookie> FACTORY = io.vertx.lang.groovy.Factories.createFactory() {
    io.vertx.ext.apex.addons.Cookie arg -> new Cookie(arg);
  };
}
