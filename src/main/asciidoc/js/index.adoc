...
a clustered session store they can be any basic type, or `link:../../vertx-core/js/jsdoc/buffer-Buffer.html[Buffer]`, `link:../../vertx-core/js/jsdoc/json_object-JsonObject.html[JsonObject]`,
`link:../../vertx-core/js/jsdoc/json_array-JsonArray.html[JsonArray]` or a serializable object, as the values have to serialized across the cluster.
...
To create an auth handler you need an instance of `link:../../vertx-auth-service/js/jsdoc/auth_service-AuthService.html[AuthService]`. Auth service is
...
When the basic auth handler receives this information, it calls the configured `link:../../vertx-auth-service/js/jsdoc/auth_service-AuthService.html[auth service]`
...
var authService = ShiroAuthService.create(vertx, 'PROPERTIES', authConfig);
var basicAuthHandler = BasicAuthHandler.create(authService);
