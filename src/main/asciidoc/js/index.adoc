...
To enable sessions in your application you make sure you have a link:jsdoc/session_handler-SessionHandler.html[`SessionHandler`]
...
link:jsdoc/local_session_store-LocalSessionStore.html#localSessionStore[`LocalSessionStore.localSessionStore`].

Here are some examples of creating a link:jsdoc/local_session_store-LocalSessionStore.html[`LocalSessionStore`]

[source,java]
----
var LocalSessionStore = require("vertx-apex-core-js/local_session_store");
var store1 = LocalSessionStore.localSessionStore(vertx);
var store2 = LocalSessionStore.localSessionStore(vertx, "myapp3.sessionmap");
var store3 = LocalSessionStore.localSessionStore(vertx, "myapp3.sessionmap", 10000);
...
[source,java]
----
var ClusteredSessionStore = require("vertx-apex-core-js/clustered_session_store");
var store1 = ClusteredSessionStore.clusteredSessionStore(vertx);
var store2 = ClusteredSessionStore.clusteredSessionStore(vertx, "myclusteredapp3.sessionmap");
...
----
var ClusteredSessionStore = require("vertx-apex-core-js/clustered_session_store");
var SessionHandler = require("vertx-apex-core-js/session_handler");
var Router = require("vertx-apex-core-js/router");
var CookieHandler = require("vertx-apex-core-js/cookie_handler");
var store = ClusteredSessionStore.clusteredSessionStore(vertx);
var sessionHandler = SessionHandler.sessionHandler(store);
...
var BasicAuthHandler = require("vertx-apex-addons-js/basic_auth_handler");
var config = {
};
config.Java.type("io.vertx.ext.auth.PropertiesAuthRealmConstants").PROPERTIES_PROPS_PATH_FIELD = "classpath:test-auth.properties";
var authService = AuthService.create(vertx, config);
var basicAuthHandler = BasicAuthHandler.basicAuthHandler(authService);
...
principal with link:jsdoc/session-Session.html#isLoggedIn[`isLoggedIn`] and link:jsdoc/session-Session.html#getPrincipal[`getPrincipal`].
...
var LocalSessionStore = require("vertx-apex-core-js/local_session_store");
var AuthService = require("vertx-auth-js/auth_service");
var RedirectAuthHandler = require("vertx-apex-addons-js/redirect_auth_handler");
var FormLoginHandler = require("vertx-apex-addons-js/form_login_handler");
var StaticServer = require("vertx-apex-addons-js/static_server");
router.route().handler(CookieHandler.cookieHandler());
router.route().handler(SessionHandler.sessionHandler(LocalSessionStore.localSessionStore(vertx)));
var authService = AuthService.createEventBusProxy(vertx, "acme.authservice");
var redirectAuthHandler = RedirectAuthHandler.redirectAuthHandler(authService);
...
----
var RedirectAuthHandler = require("vertx-apex-addons-js/redirect_auth_handler");
var managerAuthHandler = RedirectAuthHandler.redirectAuthHandler(authService);
managerAuthHandler.addRole("manager").addRole("admin");
router.route("/private/managers").handler(managerAuthHandler);
var settingsAuthHandler = RedirectAuthHandler.redirectAuthHandler(authService);
...
link:jsdoc/static_server-StaticServer.html[`StaticServer`].
...
link:jsdoc/static_server-StaticServer.html#setMaxAgeSeconds[`setMaxAgeSeconds`] if required.
...
If handling of cache headers is not required, it can be disabled with link:jsdoc/static_server-StaticServer.html#setCachingEnabled[`setCachingEnabled`].
...
link:jsdoc/static_server-StaticServer.html#setFilesReadOnly[`setFilesReadOnly`].

To enable the maximum number of entries that can be cached in memory at any one time you can use
link:jsdoc/static_server-StaticServer.html#setMaxCacheSize[`setMaxCacheSize`].

To configure the expiry time of cache entries you can use link:jsdoc/static_server-StaticServer.html#setCacheEntryTimeout[`setCacheEntryTimeout`].
...
This can be configured with link:jsdoc/static_server-StaticServer.html#setIndexPage[`setIndexPage`].

=== Changing the web root

By default static resources will be served from the directory `webroot`. To configure this use
link:jsdoc/static_server-StaticServer.html#setWebRoot[`setWebRoot`].

=== Serving hidden files

By default the serve will serve hidden files (files starting with `.`).

If you do not want hidden files to be served you can configure it with link:jsdoc/static_server-StaticServer.html#setIncludeHidden[`setIncludeHidden`].

=== Directory listing

The server can also perform directory listing. By default directory listing is disabled. To enabled it use
link:jsdoc/static_server-StaticServer.html#setDirectoryListing[`setDirectoryListing`].
...
link:jsdoc/static_server-StaticServer.html#setDirectoryTemplate[`setDirectoryTemplate`].
...
[source,java]
----
var CorsHandler = require("vertx-apex-addons-js/cors_handler");
router.route().handler(CorsHandler.cors("vertx\\.io").allowedMethod(Java.type("io.vertx.core.http.HttpMethod").GET));
...
link:jsdoc/path_template_handler-PathTemplateHandler.html[`PathTemplateHandler`].
...
[source,java]
----
var HandlebarsTemplateEngine = require("vertx-apex-addons-js/handlebars_template_engine");
var PathTemplateHandler = require("vertx-apex-addons-js/path_template_handler");
var engine = HandlebarsTemplateEngine.create();
var handler = PathTemplateHandler.templateHandler(engine);
...
Instead, the link:jsdoc/routing_context-RoutingContext.html#contextData[`contextData`] is available in the template.
...
Apex includes a handler link:jsdoc/logger-Logger.html[`Logger`] that you can use to log HTTP requests.
...
Apex includes the handler link:jsdoc/favicon-Favicon.html[`Favicon`] especially for serving favicons.
...
This is configured using an instance of link:jsdoc/timeout-Timeout.html[`Timeout`].
...
[source,java]
----
var Timeout = require("vertx-apex-addons-js/timeout");
