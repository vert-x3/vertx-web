...
To enable sessions in your application you make sure you have a link:groovydoc/io/vertx/groovy/ext/apex/core/SessionHandler.html[`SessionHandler`]
...
link:groovydoc/io/vertx/groovy/ext/apex/core/LocalSessionStore.html#localSessionStore(io.vertx.core.Vertx,%20java.lang.String,%20long)[`LocalSessionStore.localSessionStore`].

Here are some examples of creating a link:groovydoc/io/vertx/groovy/ext/apex/core/LocalSessionStore.html[`LocalSessionStore`]

[source,java]
----
import io.vertx.groovy.ext.apex.core.LocalSessionStore
def store1 = LocalSessionStore.localSessionStore(vertx);
def store2 = LocalSessionStore.localSessionStore(vertx, "myapp3.sessionmap");
def store3 = LocalSessionStore.localSessionStore(vertx, "myapp3.sessionmap", 10000);
...
Here are some examples of creating a link:groovydoc/io/vertx/groovy/ext/apex/core/ClusteredSessionStore.html[`ClusteredSessionStore`]

[source,java]
----
import io.vertx.groovy.ext.apex.core.ClusteredSessionStore
def store1 = ClusteredSessionStore.clusteredSessionStore(vertx);
def store2 = ClusteredSessionStore.clusteredSessionStore(vertx, "myclusteredapp3.sessionmap");
...
You'll also need to include a link:groovydoc/io/vertx/groovy/ext/apex/core/CookieHandler.html[`CookieHandler`] as the session handler uses cookies to
lookup the session. The cookie handler should be before the session handler when routing.

[source,java]
----
import io.vertx.groovy.ext.apex.core.ClusteredSessionStore
import io.vertx.groovy.ext.apex.core.SessionHandler
import io.vertx.groovy.ext.apex.core.Router
import io.vertx.groovy.ext.apex.core.CookieHandler
def store = ClusteredSessionStore.clusteredSessionStore(vertx);
def sessionHandler = SessionHandler.sessionHandler(store);
...
In your handlers you an access the session instance with link:groovydoc/io/vertx/groovy/ext/apex/core/RoutingContext.html#session()[`session`].

You put data into the session with link:groovydoc/io/vertx/groovy/ext/apex/core/Session.html#put(java.lang.String,%20java.lang.Object)[`put`],
you get data from the session with link:groovydoc/io/vertx/groovy/ext/apex/core/Session.html#get(java.lang.String)[`get`], and you remove
data from the session with link:groovydoc/io/vertx/groovy/ext/apex/core/Session.html#remove(java.lang.String)[`remove`].
...
You can manually destroy a session using link:groovydoc/io/vertx/groovy/ext/apex/core/Session.html#destroy()[`destroy`]. This will remove the session
...
import io.vertx.groovy.ext.apex.addons.BasicAuthHandler
def config = [:];
...
def basicAuthHandler = BasicAuthHandler.basicAuthHandler(authService);
...
principal with link:groovydoc/io/vertx/groovy/ext/apex/core/Session.html#isLoggedIn()[`isLoggedIn`] and link:groovydoc/io/vertx/groovy/ext/apex/core/Session.html#getPrincipal()[`getPrincipal`].

If you want to cause the user to be logged out you can call link:groovydoc/io/vertx/groovy/ext/apex/core/Session.html#logout()[`logout`].
...
To use redirect auth you configure an instance of link:groovydoc/io/vertx/groovy/ext/apex/addons/RedirectAuthHandler.html[`RedirectAuthHandler`] instead of a
basic auth handler.

You will also need to setup handlers to serve your actual login page, and a handler to handle the actual login itself.
To handle the login we provide a prebuilt handler link:groovydoc/io/vertx/groovy/ext/apex/addons/FormLoginHandler.html[`FormLoginHandler`] for the purpose.
...
import io.vertx.groovy.ext.apex.core.LocalSessionStore
import io.vertx.groovy.ext.auth.AuthService
import io.vertx.groovy.ext.apex.addons.RedirectAuthHandler
import io.vertx.groovy.ext.apex.addons.FormLoginHandler
import io.vertx.groovy.ext.apex.addons.StaticServer
router.route().handler(CookieHandler.cookieHandler());
router.route().handler(SessionHandler.sessionHandler(LocalSessionStore.localSessionStore(vertx)));
def authService = AuthService.createEventBusProxy(vertx, "acme.authservice");
def redirectAuthHandler = RedirectAuthHandler.redirectAuthHandler(authService);
...
----
import io.vertx.groovy.ext.apex.addons.RedirectAuthHandler
def managerAuthHandler = RedirectAuthHandler.redirectAuthHandler(authService);
managerAuthHandler.addRole("manager").addRole("admin");
router.route("/private/managers").handler(managerAuthHandler);
def settingsAuthHandler = RedirectAuthHandler.redirectAuthHandler(authService);
...
link:groovydoc/io/vertx/groovy/ext/apex/addons/StaticServer.html[`StaticServer`].
...
link:groovydoc/io/vertx/groovy/ext/apex/addons/StaticServer.html#setMaxAgeSeconds(long)[`setMaxAgeSeconds`] if required.
...
If handling of cache headers is not required, it can be disabled with link:groovydoc/io/vertx/groovy/ext/apex/addons/StaticServer.html#setCachingEnabled(boolean)[`setCachingEnabled`].
...
link:groovydoc/io/vertx/groovy/ext/apex/addons/StaticServer.html#setFilesReadOnly(boolean)[`setFilesReadOnly`].

To enable the maximum number of entries that can be cached in memory at any one time you can use
link:groovydoc/io/vertx/groovy/ext/apex/addons/StaticServer.html#setMaxCacheSize(int)[`setMaxCacheSize`].

To configure the expiry time of cache entries you can use link:groovydoc/io/vertx/groovy/ext/apex/addons/StaticServer.html#setCacheEntryTimeout(long)[`setCacheEntryTimeout`].
...
This can be configured with link:groovydoc/io/vertx/groovy/ext/apex/addons/StaticServer.html#setIndexPage(java.lang.String)[`setIndexPage`].

=== Changing the web root

By default static resources will be served from the directory `webroot`. To configure this use
link:groovydoc/io/vertx/groovy/ext/apex/addons/StaticServer.html#setWebRoot(java.lang.String)[`setWebRoot`].

=== Serving hidden files

By default the serve will serve hidden files (files starting with `.`).

If you do not want hidden files to be served you can configure it with link:groovydoc/io/vertx/groovy/ext/apex/addons/StaticServer.html#setIncludeHidden(boolean)[`setIncludeHidden`].

=== Directory listing

The server can also perform directory listing. By default directory listing is disabled. To enabled it use
link:groovydoc/io/vertx/groovy/ext/apex/addons/StaticServer.html#setDirectoryListing(boolean)[`setDirectoryListing`].
...
link:groovydoc/io/vertx/groovy/ext/apex/addons/StaticServer.html#setDirectoryTemplate(java.lang.String)[`setDirectoryTemplate`].
...
Apex includes a handler link:groovydoc/io/vertx/groovy/ext/apex/addons/CorsHandler.html[`CorsHandler`] that handles the CORS protocol for you.

Here's an example:

[source,java]
----
import io.vertx.groovy.ext.apex.addons.CorsHandler
router.route().handler(CorsHandler.cors("vertx\\.io").allowedMethod(io.vertx.core.http.HttpMethod.GET));
...
Template engines are described by link:groovydoc/io/vertx/groovy/ext/apex/addons/TemplateEngine.html[`TemplateEngine`]. In order to render a template
link:groovydoc/io/vertx/groovy/ext/apex/addons/TemplateEngine.html#render(io.vertx.ext.apex.core.RoutingContext,%20java.lang.String,%20io.vertx.core.Handler)[`render`] is used.

The simplest way to use templates is not to call the template engine directly but to use the
link:groovydoc/io/vertx/groovy/ext/apex/addons/PathTemplateHandler.html[`PathTemplateHandler`].
...
[source,java]
----
import io.vertx.groovy.ext.apex.addons.HandlebarsTemplateEngine
import io.vertx.groovy.ext.apex.addons.PathTemplateHandler
def engine = HandlebarsTemplateEngine.create();
def handler = PathTemplateHandler.templateHandler(engine);
...
When using the link:groovydoc/io/vertx/groovy/ext/apex/addons/MVELTemplateEngine.html[`MVEL template engine`], it will by default look for
templates with the `.templ` extension if no extension is specified in the file name.

The routing context (link:groovydoc/io/vertx/groovy/ext/apex/core/RoutingContext.html[`RoutingContext`] is available
...
When using the link:groovydoc/io/vertx/groovy/ext/apex/addons/JadeTemplateEngine.html[`Jade template engine`], it will by default look for
...
When using the link:groovydoc/io/vertx/groovy/ext/apex/addons/HandlebarsTemplateEngine.html[`Handlebars template engine`], it will by default look for
...
Instead, the link:groovydoc/io/vertx/groovy/ext/apex/core/RoutingContext.html#contextData()[`contextData`] is available in the template.
...
When using the link:groovydoc/io/vertx/groovy/ext/apex/addons/ThymeleafTemplateEngine.html[`Thymeleaf template engine`], it will by default look for
...
The handler is link:groovydoc/io/vertx/groovy/ext/apex/addons/ErrorHandler.html[`ErrorHandler`]. To use the error handler just set it as a
failure handler for any paths that you want covered.

== Request logger

Apex includes a handler link:groovydoc/io/vertx/groovy/ext/apex/addons/Logger.html[`Logger`] that you can use to log HTTP requests.
...
Apex includes the handler link:groovydoc/io/vertx/groovy/ext/apex/addons/Favicon.html[`Favicon`] especially for serving favicons.
...
This is configured using an instance of link:groovydoc/io/vertx/groovy/ext/apex/addons/Timeout.html[`Timeout`].
...
[source,java]
----
import io.vertx.groovy.ext.apex.addons.Timeout
