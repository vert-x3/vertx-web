...
# Generated from io.vertx.ext.apex.handler.CorsHandler
module VertxApex
  #  A handler which implements server side http://www.w3.org/TR/cors/[CORS] support for Apex.
  class CorsHandler
    # @private
    # @param j_del [::VertxApex::CorsHandler] the java delegate
    def initialize(j_del)
      @j_del = j_del
    end
    # @private
    # @return [::VertxApex::CorsHandler] the underlying java delegate
    def j_del
      @j_del
    end
    ...
    # @return [::VertxApex::CorsHandler] the handler
    def self.create(allowedOriginPattern=nil)
      if allowedOriginPattern.class == String && !block_given?
        return ::VertxApex::CorsHandler.new(Java::IoVertxExtApexHandler::CorsHandler.java_method(:create, [Java::java.lang.String.java_class]).call(allowedOriginPattern))
      end
